...
#imports
import numpy as np
import functionsSphericalHarmonic as f
import matplotlib.pyplot as plt

class SphericalHarmonic:
    def __init__(self, initr,  omega, vmult = 1):
        self.omega = omega
        self.initr = initr
        self.vmult = vmult
    ...
    def runLeapfrog(self, dt, tmax, printBool = False, printLim = None):
        ...
        x, v = f.calculateInit(self.initr, self.omega, self.vmult)
        

        a=f.getAcceleration(x,self.omega)
        ...
        initEnergy = [f.getKineticEnergy(x,v),f.getPotentialEnergy(x, self.omega)]
        ...
        while (time<=tmax):
            localxva = f.step_leapfrog(x,v,a,dt,self.omega)
            x = localxva[0]
            v = localxva[1]
            a = localxva[2]
            time = float(step)*dt
            ...
            energy = [f.getKineticEnergy(x,v),f.getPotentialEnergy(x,self.omega)]
    ...
    def plot(self, xvar, yvar, equal = False, legend = False, endPlot = True, legendLabel = None):
        ...
        plt.xlabel(xvar)
        plt.ylabel(yvar)
        plt.title(xvar+' vs '+yvar+' , dt == '+str(self.rundt)+' , omega == '+str(self.omega)+', tmax == '+str(np.round(self.runtmax, 3))+', initr == '+str(self.initr)+', vmult== '+str(self.vmult))
        if equal:
            plt.axis('equal')
        if legend:
            plt.legend()
